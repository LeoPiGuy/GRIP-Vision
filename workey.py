...
def extra_processing(pipeline):
    ...
    for value, contour in enumerate(pipeline.filter_contours_output):
        if value == 0:
            x, y, w, h = cv2.boundingRect(contour)
            print("X:" + str(x + w / 2))  # X and Y are coordinates of the top-left corner of the bounding box
            print("Y:" + str(y + h / 2)) 
            print("L:" + str(w))
            print("H:" + str(h))
        # If the file name exists, write a JSON string into the file.
            with open("infoo.json", 'w') as f:
                json.dump(cv2.boundingRect(contour), f)
